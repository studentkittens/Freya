...
Alle Methoden im nächsten Block lösen ein ,,options'' Event aus.
\begin{verbatim}
...
Siehe daher AbstractItemGenerator für eine genaue Erklärung.
\begin{verbatim}
...
Die Statistics Klasse ist Wrapper für mpd\_stats,
implementiert gemäß \url{http://www.musicpd.org/doc/libmpdclient/stats\_8h.html}
folgende Funktionen:
\begin{verbatim}
...
Die Playlist Klasse ist Wrapper für die mpd\_playlist Struktur,
...
Sie erlaubt den Einsatz des Proxy-Patterns. Andere Klassen können sich so als Client ,,ausgeben''.
...
Der Hauptnutznieser dieser Klasse ist der Databasebrowser, bzw. den dahinter gelagerten Cache Songs und Verzeichnisse gleich zu behandeln (vgl. Composite Pattern).
...
% Klassen diagramm aus Doxygen hierhin.

Diese abstrakte Klasse erlaubt abgeleiteten Klasse ähnlich zum BaseClient eigene Kommandos zu implementieren.
Dies geschieht indem die abstrakte Klasse einen Referenz auf den BaseClient im Konstruktor erwartet und speichert.
...
AbstractClientExtension wird in diesem Entwurf von MPD::Playlist und MPD::AudioOutput benutzt.
Man kann allerdings darüber diskutieren dass dieses Interface dem Model zuviel Logik mitgibt. 
